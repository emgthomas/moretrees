...
########## Compare VI and MCMC credible intervals/variance ##############
nonzero_sd <- function(x){
  if(sum(x!=0)<=1) return(0)
  return(sd(x[x!=0]))
} 
sd_VI <- matrix(nrow=pL,ncol=nsims)
sd_MCMC <- matrix(nrow=pL,ncol=nsims)
sd_nonzero_mat <- matrix(nrow=0,ncol=4)
for(sim in 1:nsims){
  # load VI results
  res_VI <- paste0("./data_example_results/comparison_vi_results",sim,".Rdata")
  load(res_VI)
  VI_params <- out_vi$VI_params
  # SD of individual betas
  sd_VI[,sim] <- beta.sd.calc(mu_gamma=VI_params$mu_gamma,
                              sigma2_gamma=VI_params$sigma2_gamma,
                              u_s=VI_params$u_s,
                              ancestors=ancestors,
                              pL=pL,p=p)
  sd_MCMC[,sim] <- apply(beta_mcmc[[sim]][,1:pL],2,FUN=sd)
  
  # SD of non-zero gammas
  which_nonzero <- which(p_s_mcmc[,sim]>=cutoff)
  sd_VI_nonzero <- sqrt(VI_params$sigma2_gamma[which_nonzero])
  sd_MCMC_nonzero <- apply(gamma_mcmc[[sim]][,1:p],2,FUN=nonzero_sd)[which_nonzero]
  sd_nonzero_mat <- rbind(sd_nonzero_mat,
                          cbind(sd_VI_nonzero,
                                sd_MCMC_nonzero,
                                which_nonzero,
                                rep(sim,length(sd_VI_nonzero))))
  row.names(sd_nonzero_mat) <- NULL
}
...
################## Compare SD for betas ###################
{
  ...
  ratio_sims <- tapply(sd_dat$ratio,
                       sd_dat$sim,
                       median)
  cat(paste0("\n\nThe VI estimate of standard error of beta_v was ",
...
############## Compare SD for non-zero components #############
{
  ...
  ratio_sims <- tapply(sd_nonzero_dat$ratio,
                       sd_nonzero_dat$sim,
                       mean)
  cat(paste0("\n\nOn average, VI estimate of standard error for nonzero gammas was ",
