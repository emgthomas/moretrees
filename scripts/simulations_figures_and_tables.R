...
for(i in (1:nrow(params))){
  ...
  outfile_reached_max <- paste0(path_file,"reached_max_n",nsamp,"_beta",whichbeta,".csv")
  reached_max <- read.csv(file=outfile_reached_max,header=T,row.names=NULL)
  # extract results
  outfile_beta <- paste0(path_file,"betasims_n",nsamp,"_beta",whichbeta,".csv")
...
# simulation 5 did not converge 27 times
...
pdf(file="figures_and_tables/figure3.pdf",width=9,height=8)
rmse_plot
dev.off()
...
pdf(file="figures_and_tables/figureA2_a.pdf",width=12,height=6)
plot_ARI_rmse
dev.off()
...
pdf(file="figures_and_tables/figureA2_b.pdf",width=12,height=6)
plot_nclust_rmse
dev.off()
